...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/xamarin.ios-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/xamarin.ios-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '') {
                ...
                if (fs.existsSync('../xamarin.ios-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/xamarin.ios-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../xamarin.ios-docs/` + changedFileName, `../../../gitlabRepo/xamarin.ios-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/xamarin.ios-docs/')) {
                            shelljs.cp('-rf', `../xamarin.ios-docs/` + changedFileName, `../../../gitlabRepo/xamarin.ios-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/xamarin.ios-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/xamarin.ios-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/xamarin.ios-docs`);
